...
static int wfd_vidbuf_buf_init(struct vb2_buffer *vb)
{
	...
	struct mem_region mregion;
	/*htc start: clean_li added for klocwork issue 4442*/
	if (!minfo)
		return -ENODEV;
	/*htc end: clean_li added for klocwork issue 4442*/
	mregion.fd = minfo->fd;
	mregion.offset = minfo->offset;
	mregion.cookie = (u32)vb;
	/*TODO: should be fixed in kernel 3.2*/
	/*htc start: clean_li added for klocwork issue 4447*/
	if (inst)
	/*htc end: clean_li added for klocwork issue 4447*/
		...
		struct v4l2_buffer *b)
{
	...
	if (!minfo) {
		...
		if (copy_from_user(&minfo_entry->minfo, (void *)b->reserved,
			...
			/*htc start: clean_li added for klocwork issue 5395*/
			kfree(minfo_entry);
			/*htc end: clean_li added for klocwork issue 5395*/
			return -EINVAL;
		}
